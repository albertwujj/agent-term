...
- User docs under `docs/` name capabilities and never narrate the UI (labels, hints, key lists, messages); the rule and its test are in [docs/dev/development.md](docs/dev/development.md#user-docs).
...
- `src/stream/` is the client for [agent-stream-hub](https://github.com/albertwujj/agent-stream-hub), the phone viewer. The protocol (`stream.md`) and the hub setup (its README) live in that repo, which the comments reference as a sibling checkout; the terminal's side is `hubUrl` in `~/.agent-term/config.json`; the hub secret is entered on the phone.
