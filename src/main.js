...
// IPC handlers
ipcMain.on('pty-input', (event, data, promptSnapshot) => {
  ...
  // renderer snapshot is used only to reconcile an @ completion in the
  // same input row. It never changes the bytes sent to the PTY.
  if (promptCapture && !promptCapture.isLocked()) {
    promptCapture.handleInput(typeof data === 'string' ? data : String(data), promptSnapshot);
  }
});
