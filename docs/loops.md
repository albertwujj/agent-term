...
A clone is best placed in `ai/` in the project; [placement](conventions.md) covers the other places it can live. A loop is commanded from the prompt by naming one of its instruction files, such as `@produce-r` for a review.
