...
A long run the agent starts (CI, a heavy test suite, a deploy) puts a standard session in a bind: the agent either sits watching it, blocking the terminal, or ends its turn and the finish arrives to nobody, the result sitting unhandled until you notice.
