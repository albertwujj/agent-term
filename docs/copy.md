...
**From the terminal.** What the agent prints in the terminal is laid out for the terminal: prose wrapped at the column width, a mark at the start of each message, borders around a boxed prompt. `Ctrl/Cmd+C` on a selection copies it as message text: the marks and borders drop out, each wrapped paragraph becomes one line while a line that ends short of the right edge keeps its break, list items stay one per line, and punctuation, paths, and `file:line` references stay as printed. `Ctrl/Cmd+Shift+C` copies the selection as it appears on screen, the copy to take for code.
