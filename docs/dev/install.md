...
| Phone view config | `~/.agent-term/config.json` | `~/.agent-term/config.json` inside the WSL distro |
...
- **The phone view's indicator says streaming is disabled**: `hubUrl` goes in `~/.agent-term/config.json` inside the WSL distro.
