...
The phone view is self-hosted and opt-in: you run the relay on a machine you own, and everything travels over plain outbound HTTPS, requiring no inbound ports or VPN. Once the relay runs, point this terminal at it: its URL goes in `~/.agent-term/config.json` as `hubUrl`, and the next window streams. Ask your agent:
