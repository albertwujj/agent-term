...
The same channel lets a capable agent do more than its CLI can alone: it prints, and the host acts. A CLI does not own the window, so when Claude Code publishes a design mock it can only print the URL and go around the terminal, opening your browser on it. With a host that reacts, the printed line alone is enough: the viewer opens right in the window, with placement and sizing optimized for the situation. The rest works the same way: the agent puts a plan in front of you, hands you a review, starts a job that reports back, and reaches you on your phone.
