...
| Its plans are append-only text. | Ask for the plan as a markdown file and click its name: the doc opens rendered, you **[comment on any passage and write in the rendered page](docs/plan.md)**, and the agent maintains the source. The plan is a file in your repo, versioned and there for later sessions. |
