...
| Everything the agent prints (a diff, a plan, a claim, a link) is dead text you can read but not act on. | **Select any of it and comment**; the agent makes the change. A click opens whatever renders (docs, reviews, images) inside the window; web links open in your browser. [Details](docs/comment.md). |
