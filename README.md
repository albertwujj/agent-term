...
The vendor desktop apps also offer a richer interface, each built around its own agent. This terminal hosts any agent beside your shell, your repo, and your tests, and it keeps the terminal's focus: one window per agent with only that session in it, full screen if you like ([native to the OS](#native-to-the-os)).
