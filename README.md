...
|---|---|
| Several agents running means identical tabs outside, walls of text inside. | Each session gets its own **[unique taskbar button and preview](docs/sessions.md)** on Windows (a Dock tile of its own on a Mac), so you tell them apart at a glance, and the picker searches instantly, down to every prompt you typed and more. |
...
The above covers the first set of features: sessions as windows with their taskbar buttons and picker, and commenting on anything the agent prints. [Existing sessions](docs/sessions.md) from before this terminal work too.
