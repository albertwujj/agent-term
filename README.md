...
3. From the clone: `npm ci` once, to install the dependencies, then `npm run start` (`npm run start:wsl` on WSL); or start from your own repo with `--prefix` pointing at the clone ([sessions](docs/sessions.md)).
