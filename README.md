...
- **Fleet coordination**: a lock / ownership / HEAD-guard convention so many agents can share one checkout without clobbering each other ([agent-lock](https://github.com/yunxin/agent-lock)). AgentTerm shows who holds the checkout as a padlock in the chrome bar: green with a check when it is this window's, the holder window's own color otherwise (filled while that window works, hollow while it waits, dashed when it has no window open). Status only; the lock's own guards do the stopping.
- **Conventions by filename**: agents bind to docs by name, resolving to the nearest such file up the directory tree (`commit-message.md`, the kits' config; [agent-lock's `coding-guide.md` handoff](https://github.com/yunxin/agent-lock/blob/main/proceed-by-branching.md) shows the rule). No central config.
