...
| Its plans are append-only text. | Ask for the plan as a markdown file and click its name. The doc opens rendered, and the rendered page is where you work: **[comment on any passage, or write in it directly](docs/plan.md)**; the agent maintains the source, answers in a thread on the passage, and the margin marks what it changed. |
